...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | OGC小站")[0];
}
...
$.ajax({
  ...
  data: {
    key: 'EUEBZ-SIHC7-F3MXQ-PJ2DT-BT73J-UYBKH',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  let dist = getDistance(110.288357, 25.778331, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by ogcfun🥝 %c 你正在访问OGCの小站",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| OGC小站") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('OGC的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
document.addEventListener('visibilitychange', function () {
  if (document.hidden) {
    //离开当前页面时标签显示内容
    document.title = 'Σ( ° △ °|||)︴不要离开我';
    clearTimeout(titleTime);
  } else {
    //返回当前页面时标签显示内容
    document.title = 'ξ( ✿＞◡❛) 终于等到你';
    //两秒后变回正常标题
